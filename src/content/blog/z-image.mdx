...
**Note:** You are likely to see many warnings when running this command. They are harmless and can be ignored. Model compilation might even fail the first time, rerunning the `train` command will fix it.
...
| Epoch | What to Check |
|-------|---------------|
| 1-10 | Loss should start decreasing |
| 10-30 | Sample images should show the style emerging |
| 30-100 | Check for overfitting (samples look too much like training images) |
| 100+ | If loss is still decreasing, consider more epochs |
...
> ***Note:*** Do not give up too early! Z-Image LoRA can require a lot of epochs before you reach the desired effect.
