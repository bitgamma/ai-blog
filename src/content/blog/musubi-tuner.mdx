...
This guide walks you through training a custom LoRA (Low-Rank Adaptation) using musubi-tuner on AMD Strix Halo GPUs. We'll use Flux 2 Klein 4B as an example but the guide applies to Flux 2 Klein 9B and can be adapted to other models as well.

## Hardware Requirements

This guide is optimized for AMD GPUs with the following recommendations:

| Component | Minimum | Recommended | Notes |
|-----------|---------|-------------|-------|
| GPU | AMD Radeon RX 7900 XTX | AMD Radeon RX 7900 XTX or better | RDNA3 architecture required |
| RAM | 64 GB | 128 GB | Strix Halo |
| OS  | Any Linux | Linux (Arch/Ubuntu) | ROCm is installed automatically by uv |

**Note on RAM:** Training with 64GB is possible but may require smaller batch sizes or lower resolutions. 128GB provides the most comfortable experience.
...
- `network_dim`: Dimension of the LoRA (16-64, higher = more capacity but slower)
- `network_alpha`: Should match network_dim for best results
...
- Reduce `batch_size` in `config.toml` (try 2 or 1)
- Reduce `resolution` (try 768x768)
- Reduce `network_dim` (try 8 instead of 16)
- Reduce `max_data_loader_n_workers` (try 1 or 0)
