...
This guide walks you through training a custom LoRA (Low-Rank Adaptation) using musubi-tuner on AMD Strix Halo APUs. This guide works as-is for both Flux.2 Klein 4B and 9B. For other models small adjustments will be needed they and will be addressed in future guides.
...
Download the [musubi wrapper script](../../public/musubi-tuner/musubi.sh) and copy it where you want. This is a small script I created to simply the procedure. The script is easy to read so if you are curious have a look at what it does!

Now `cd` to the directory where the script. From there you will need to docs
...
# Set the path to the first shard of the text_encoder (e.g: model-00001-of-00002.safetensors or and model-00001-of-00004.safetensors)
export TEXT_ENCODER="/path/to/te/model-00001-of-00002.safetensors"

# Set the path to the vae (eg: ae.safetensors)
...
A good dataset is crucial for training a useful LoRA. Here are some technical advices and rules of thumb. Be ready to experiment:
...
- **Resolution:** Aim for 1024x1024. Using a single resolutions for all images can reduce the number of batches, which is good
- **Aspect ratio:** Square images work best but other ratios will work too

As a rule of thumb anywhere from 20-200 images will work. The quality of the images is more important that the number.
...
- For styles: describring the scene but not the style works. I had good results with just empty captions.
- For characters: a trigger word + a short description seems to work well.

### Editing the Dataset Config

In the project directory you will find a `dataset.toml` file. While usable as-is here is the explaination of some of its parameters:
...
- `save_state`: Saves the training state with each checkpoint. This allows stopping and resuming training. Consumes more disk space and VRAM
...
If you have stop the training, or feel that even after finishing the LoRA is undertrained, you can resume training if you set `save_state` to true (which is the default) in `training.toml`. To resume only run:
...
However the best way to tell is to test the checkpoints that look promising with the distilled model using [ComfyUI](../comfyui/). I have found that often the last checkpoint is overtrained and prefer to use 2-4 epoch older checkpoints.
